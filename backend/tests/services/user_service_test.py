    ...
    def test_login_EmailLogin(self,manage_db_transaction, mock_insert_query):
        ...
        async def async_test():
            with patch('services.user_service.read_query') as mock_read_query:
                ...
                self.assertEqual(dict(access_token=token, token_type="bearer",is_admin=False,response_data= response),result_emailLogin)
    ...
    def test_login_UsernameLogin(self,manage_db_transaction, mock_insert_query):
        ...
        async def async_test():
            with patch('services.user_service.read_query') as mock_read_query:
                ...
                responses.get(url = 'https://api.chatengine.io/users/me/',status = 200,json={'username':USERNAME,'secret': PASSWORD})
                ...
                self.assertEqual(dict(access_token=token, token_type="bearer",is_admin=True,response_data= response), result_usernameLogin)

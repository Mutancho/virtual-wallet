...
async def create(user:RegisterUser) -> RegisterUser:
    ...
    (user.username, hashed,user.title,user.first_name,user.last_name,user.gender,user.date_of_birth,user.address,user.email,user.phone_number,user.photo_selfie,user.identity_document))
...
async def login(credentials: EmailLogin | UsernameLogin):
    if isinstance(credentials, EmailLogin):
        data = await read_query('''SELECT id,email FROM users WHERE email = %s''',
                                (credentials.email,))
    if isinstance(credentials, UsernameLogin):
        data = await read_query('''SELECT id,username FROM users WHERE username = %s''',
...
async def update(id:int,user:UpdateUser):
    ...
    (merged.new_password, email_verified, merged.email, merged.phone_number, merged.first_name, merged.last_name,
     merged.address, merged.two_factor_method,merged.title, merged.gender, merged.photo_selfie, merged.identity_document, id))
...
async def exists_by_username_email_phone(user):
    data = await read_query('''SELECT u.username,u.email,u.phone_number FROM users as u WHERE u.username = %s or u.email = %s or u.phone_number = %s ''',
                            (user.username,user.email,user.phone_number))

    return len(data) > 0

async def check_exists_by_email_phone_for_updating(id:int,user: UpdateUser):
    data = await read_query('''SELECT u.email,u.phone_number FROM users as u WHERE (u.email = %s or u.phone_number = %s ) and u.id <> %s''',
